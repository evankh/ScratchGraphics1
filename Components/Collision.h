#ifndef __EKH_SCRATCH_GRAPHICS_1_COLLISION_COMPONENT__
#define __EKH_SCRATCH_GRAPHICS_1_COLLISION_COMPONENT__
...
class CollisionComponent {
	friend bool collides(CollisionComponent*, CollisionComponent*);
protected:
	BoundsType mType;
	CollisionComponent(BoundsType type) :mType(type) {};
public:
	...
	virtual CollisionComponent* copy() const = 0;
};
...
class Collisionless :public CollisionComponent {
...
public:
	Collisionless() :CollisionComponent(BoundsType::COLLISIONLESS) {};
};

class BoundingSphere :public CollisionComponent {
...
private:
	...
	float mRadius;
public:
	BoundingSphere(glm::vec3 center, float radius) :CollisionComponent(BoundsType::SPHERE) { mCenter = center; mRadius = radius; };
	virtual void debugDraw();
...
public:
	AABB() :CollisionComponent(BoundsType::AA_BOX) {};
	AABB(glm::vec3 min, glm::vec3 max) :CollisionComponent(BoundsType::AA_BOX), mMin(min), mMax(max) {}
	virtual void debugDraw();
	virtual void rotate(glm::vec3 axis, float degrees) {};
	virtual void translate(glm::vec3 dxyz);
...
*/
class CollisionPlane :public CollisionComponent {
	friend bool collides(CollisionPlane*, BoundingSphere*);
	friend bool collides(CollisionPlane*, AABoundingCylinder*);
	friend bool collides(CollisionPlane*, AABB*);
	friend bool collides(CollisionPlane*, ArbitraryBoundingCylinder*);
...
public:
	CollisionPlane(glm::vec3 position, glm::vec3 normal) :CollisionComponent(BoundsType::PLANE) { mPosition = position; mNormal = normal; };
	virtual void debugDraw();
	virtual void rotate(glm::vec3 axis, float degrees);
...
#endif//__EKH_SCRATCH_GRAPHICS_1_COLLISION_COMPONENT__